...
raw_sql = """SET hive.exec.dynamic.partition=true;
...
sql = to_query_parser.replace("overwrite","").replace("partition", "").replace("create external table", "insert into")
tablas = Parser(str(sql)).tables

for tabla in tablas:
    sink = sql.find(tabla)
    determine_sink = sql[0:sink]
    if "insert" in determine_sink and "from" not in determine_sink:
        print(tabla + ": sink")
    else:
        print(tabla + ": source")
